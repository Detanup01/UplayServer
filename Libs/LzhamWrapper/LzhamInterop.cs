...
{
    internal static class LzhamInterop
    {
        ...
        static LzhamInterop()
        {
            if (Environment.OSVersion.Platform == PlatformID.Win32NT)
            {
                ...
                {
                    Trace.TraceWarning($"{nameof(LzhamWrapper)}: Failed to get executing assembly location");
                    return;
                }

                var platform = Environment.Is64BitProcess ? "x64" : "x86";
                if (!SetDllDirectory(Path.Combine(path, platform)))
                    Trace.TraceWarning($"{nameof(LzhamWrapper)}: Failed to set DLL directory to '{path}'");
            }

        }


        [DllImport(LzhamConsts.LzhamDll, CallingConvention = CallingConvention.Cdecl)]
