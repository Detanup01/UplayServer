        ...
        private void Main(object? obj)
        {
            while (!IsClosed)
            {
                try
                {
                    ...
                    Debug.WriteDebug($"unlocked at {CurrentClientNumber} end");
                }
                catch (Exception ex)
                {
                    File.AppendAllText("demux_main_ex.txt", $"M: {ex.Message}\nS: {ex.Source}\nST: {ex.StackTrace}");
                }
            }
        }
        /// <summary>
        ...
        private void StateCheck(object? obj)
        {
            while (!IsClosed)
            {
                ...
                if (ClientModifing == false)
                {
                    ...
                    {
                        File.AppendAllText("demux_state_ex.txt", $"M: {ex.Message}\nS: {ex.Source}\nST: {ex.StackTrace}");
                    }
                }
            }
        }
        ...
        void OnDataReceived(int ClientNumb, SslStream sslStream, byte[] receivedData)
        {
            ...
            switch (first)
            {
                case 0x12:
                    ...
                    Debug.PWDebug($"[DMXSERVER] {ClientNumb}: (DMX) Push \n" + req.Push);
                    Demux.PushRSP.Push(ClientNumb, req.Push);
                    Which = 0;
                    break;
                case 0x0A:
                    var req2 = Upstream.Parser.ParseFrom(receivedData);
                    Debug.PWDebug($"[DMXSERVER] {ClientNumb}: (DMX) Req \n" + req2.Request);
            ...
            {
                File.AppendAllText($"client_{ClientNumb}_rsp.log", Demux.PushRSP.Downstream.ToString() + "\n");
            ...
            {
                File.AppendAllText($"client_{ClientNumb}_rsp.log", Demux.ReqRSP.Downstream.ToString() + "\n");
