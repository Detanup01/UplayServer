...
namespace Core.DemuxResponsers
{
    public static class Demux
    {
        ...
        public static class PushRSP
        {
            ...
            {
                File.AppendAllText($"client_{ClientNumb}_push.log", push.ToString() + "\n");
                    ...
                    if (Globals.Connections.Contains(ConnectName))
                    {
                        switch (ConnectName)
                        {
                            case "download_service":
                                ...
                                File.AppendAllText($"client_{ClientNumb}_download_rsp.log", Download.Up.Downstream.ToString() + "\n");
                                rspBytes = Download.Up.Downstream.ToByteString();
                                break;
                            case "denuvo_service":
                                Denuvo.Up.UpstreamConverter(ClientNumb, data.Data);
                                File.AppendAllText($"client_{ClientNumb}_denuvo_rsp.log", Denuvo.Up.Downstream.ToString() + "\n");
                                rspBytes = Denuvo.Up.Downstream.ToByteString();
                                break;
                            case "friends_service":
                                Friends.Up.UpstreamConverter(ClientNumb, data.Data);
                                File.AppendAllText($"client_{ClientNumb}_friends_rsp.log", Friends.Up.Downstream.ToString() + "\n");
                                rspBytes = Friends.Up.Downstream.ToByteString();
                                break;
                            case "ownership_service":
                                Ownership.Up.UpstreamConverter(ClientNumb, data.Data);
                                File.AppendAllText($"client_{ClientNumb}_ownership_rsp.log", Ownership.Up.Downstream.ToString() + "\n");
                                rspBytes = Ownership.Up.Downstream.ToByteString();
                                break;
                            case "party_service":
                                Party.Up.UpstreamConverter(ClientNumb, data.Data);
                                File.AppendAllText($"client_{ClientNumb}_party_rsp.log", Party.Up.Downstream.ToString() + "\n");
                                rspBytes = Party.Up.Downstream.ToByteString();
                                break;
                            case "store_service":
                                Store.Up.UpstreamConverter(ClientNumb, data.Data);
                                File.AppendAllText($"client_{ClientNumb}_store_rsp.log", Store.Up.Downstream.ToString() + "\n");
        ...
        public static class ReqRSP
        {
            ...
            {
                File.AppendAllText($"client_{ClientNumb}_req.log", req.ToString() + "\n");
                ReqId = req.RequestId;
