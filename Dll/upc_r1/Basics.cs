...
public class Basics
{
    ...
    {
        File.AppendAllText(GetCuPath() + "\\upc_r1.log", $"{Environment.ProcessId} | {actionName} {string.Join(", ", parameters)}\n");
    }

    public static void LogReq(Uplay.Uplaydll.Req req)
    {
        File.AppendAllText(GetCuPath() + "\\upc_r1_req.log", $"{req}\n");
    }

    public static void LogRsp(Uplay.Uplaydll.Rsp rsp)
    {
        File.AppendAllText(GetCuPath() + "\\upc_r1_rsp.log", $"{rsp}\n");
    }
